...
const Discount = () => {
    ...
    const applyDiscount = async () => {
        ...
        try {
            // İndirimli fiyatları güncellemek için Promise.all kullanıyoruz
            const updatedDiscountedProducts = await Promise.all(discountedProducts.map(async (product) => {
                ...
                // API'ye güncel fiyatı gönder
                await axios.patch(`http://localhost:8080/api/products/${product.id}/update-price`, {
                    price: parseFloat(discountedPrice)
                });


                // `discountedProducts` tablosuna eski ve yeni fiyatları gönder
                const addDiscountedProduct = async (product, discountedPrice) => {
                    try {
                        const res = await axios.post('http://localhost:8080/api/discountedProducts', {
                            productId: product.id,  // Seçilen Product'ın ID'si
                            oldPrice: product.price,
                            discountedPrice: discountedPrice,
                            discountRate: discountRate,
                            discountDate: new Date().toISOString()  // Bugünün tarihi
                        });
                ...
                // Ürünü güncellenmiş fiyat ile döndür
